...
<span>
    ...
    <c:forEach var="product" items="${auxiliars}" varStatus="nr">
        <tr id="${nr.count - 1}" class="<%= rowodd ? "odd" : "even" %>">
            ...
            <td style="background-color:#ffb7b3;"><input value="Add" type="submit" class="btn btn-success" style="width:70px; margin-bottom:3px;" onclick="ajaxAddProduct('<%=request.getSession().getAttribute("place")%>', ${nr.count - 1}, '${product.name}', '${product.id}', 1);"/></td>
        </tr>
    </c:forEach>
</span>
